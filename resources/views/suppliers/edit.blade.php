        ...
        <form action="{{ route('suppliers.update', $supplier) }}" method="POST">
            ...
            <div class="mt-6">
                ...
                <textarea name="address" id="address" rows="3"
                          class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                          placeholder="Dirección completa de la empresa...">{{ old('address', $supplier->address) }}</textarea>
                @error('address')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" id="notes" rows="4"
                          class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                          placeholder="Notas, comentarios o información adicional sobre el proveedor...">{{ old('notes', $supplier->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>

            @if($supplier->equipment()->count() > 0 || $supplier->contracts()->count() > 0)
                ...
                <div class="flex">
                    <div class="ml-3">
                        ...
                        <div class="text-sm text-blue-700 mt-1">
                            Este proveedor tiene {{ $supplier->equipment()->count() }} equipo(s) y {{ $supplier->contracts()->count() }} contrato(s) asociado(s).
                        </div>
                    </div>
                </div>
            </div>
